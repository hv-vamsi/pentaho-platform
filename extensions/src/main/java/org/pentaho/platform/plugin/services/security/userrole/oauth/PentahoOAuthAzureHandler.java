...
public class PentahoOAuthAzureHandler implements IPentahoOAuthHandler {
  ...
  public List<String> getAppRoleAssignmentsForUser( String registrationId,
    ...
    try {
      var responseEntity = PentahoOAuthUtility.getInstance().getResponseEntity( registrationId + ".app-role-assignments",
              clientCredentialsToken,
              userId,
              retry,
              Map.class );
      ...
      if ( retry ) {
        return getAppRoleAssignmentsForUser( registrationId, getClientCredentialsToken( registrationId, true ), userId, false );
      }
    }
    return roles;
  }
  ...
  public void performSyncForUser( PentahoOAuthUser pentahoUser ) {
    ...
    boolean isUserAccountEnabled = isUserAccountEnabled( registrationId, clientCredentialsToken, pentahoUser.getUserId(), true );
    ...
    List<String> oauthRoleIds = getAppRoleAssignmentsForUser( registrationId, clientCredentialsToken, pentahoUser.getUserId(), true );
